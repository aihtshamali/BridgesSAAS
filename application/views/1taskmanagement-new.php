...
<?php 									for ($i=0; $i < 8; $i++) { 
									...
									<?php
										...
										$diff1 = $diff1->format("%r%a"); $diff2 = $diff2->format("%r%a");
										$y = $diff2 + $diff1;

										$y = ($y / $diff2) * 100; $daysleft = $diff->format("%r%a");
									}
									?>
			...
			<div class="col-md-1 e-width pdl-0" style="background: white;display: flex;">
				<div class="border-r showMore-arrow" style="width: 9%;padding: 1px;height: 100%;line-height: 70px;font-size:10px;background-color: #f0f0f0;"><i class="fa fa-arrow-down" aria-hidden="true" onclick="showMore()"></i></div>

			   
				<div class="custom-resources drag"></div>
				<?php if (isset($bm_tools[$counter])) { ?>
				
				
			 	 <a class="fs-11" style="color: green; margin:5px;" href="<?php echo prep_url($bm_tools[$counter]['link']);?> "> <?php echo $bm_tools[$counter]['title']; ?></a> 	
			 	 <?php } ?> 
...
//for add task
  ...
  function showMore(){
  	...
  	alert(pos);
  	// $(".moreTask").html("asldkfjaskdfnjalksdjfnalksdjfalksdjnfalksjndflaksdjnflaksdjfnalksjdfnlaskdjfnasdlkfnjasdlk");
  }
  //for edit task
</script>
