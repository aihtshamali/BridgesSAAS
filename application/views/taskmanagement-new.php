					...
					<div class="col-md-1">
						...
						<?php 
          				$options = array(1 => 1,
				                        ...
				                        4 => 4,
				                        5 => 5);
          				$classes = array('class' => 'form-control input-sm-h25 fs-11 padding-0 pull-right',
          					'id' => 'selCluster'
          					);
 ...
 -->
			...
			<div class="col-md-6 task-process">
				<div class="tabs-dv fs-10 pull-right output2-div">
					<span class="action pull-left">
					<?php if ($this->session->userdata('usertype') == 'Cordinator') {
						echo '<i class="fa fa-plus" aria-hidden="true" onClick="AddTask()"></i>';
					}?>
					</span>Task &amp; Process
				</div>
			</div>
			...
			<!-- Salman ends here -->
				...
				<div class="col-md-2 border-r fs-11 pdr-0" style="width:16.666%;height: 64px;">
					<?php if(!empty($activity['0'])) {
						//var_dump($activity['0']);
						$activity0id = $activity['0']['id'];
						$activity0title = $activity['0']['title'];
					?>
...
<!-- 
				...
				<?php if (isset($bm_tools[$tools])) { 
					...
					<div style="display:flex; height: 17px;">
			 	 		<a class="fs-11" style="color: green; margin-left:5px; width:85%; " href="<?php echo prep_url($bm_tools[$tools]['link']);?> "> <?php echo $bm_tools[$tools]['title']; ?></a>
