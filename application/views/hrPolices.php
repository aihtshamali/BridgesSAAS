...
<body>
<?php if(file_exists('uploads/hrPolices/'.$user_id.'.html')) header('Location: '.base_url().'uploads/hrPolices/'.$user_id.'.html'); ?>
<style>
...
// echo'<pre>';print_r($user);	
 if($user->hired_for_project==1){ ?>
<img src="<?php echo base_url(); ?>/assets/images/final_logo.jpg" style="max-height:75px;">  
<?php } elseif($user->hired_for_project==3) {?>  
<img src="<?php echo base_url(); ?>assets/emp_profile/card/img/bridges-analytics.png" style="max-height:100px;">  
<?php } else {?>  
<img src="<?php echo base_url(); ?>/assets/images/red-logo.png" style="max-height:75px;">  
<?php } ?>
<!-- <br><br>  -->
<div id="1" contenteditable="true">
<?php echo $this->hr_m->getHrPolicyContent('1'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('3'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('4'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('5'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('6'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('7'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('8'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('9'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('10'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('11'); ?>
</div>

<div id="13" contenteditable="true">
<?php echo $this->hr_m->getHrPolicyContent('13'); ?>
</div>
...
<?php echo $this->hr_m->getHrPolicyContent('12'); ?>
</div>
...
<script type="text/javascript">
	...
	<?php for($i=0; $i<=9; $i++){ ?>
		...
		<?php if(file_exists('uploads/empSign/'.$user_id.'-'.$i.'.png')){ ?>
				$('#signBody<?=$i;?>').html("<img src='<?php echo base_url(); ?>uploads/empSign/<?=$user_id;?>-<?=$i;?>.png' alt='Signature'/>");
		<?php } ?>
        ...
        function save<?=$i;?>()
		{
			...
			{	
				$.post('<?php echo base_url(); ?>caan/hrPolicySignSave/<?=$user_id;?>/<?=$i;?>', {imagedata: imageData}, function(response)
					{
    ...
    <?php } ?>
					...
					on: {
						blur: function( event ) {
							...
							var request = jQuery.ajax({
								url: "<?php echo base_url();?>caan/hrPolicyContentSave",
								type: "POST",
								data: {
									content : data,
		...
		function saveDoc() {
			...
			jQuery.ajax({
						url: "<?php echo base_url();?>caan/saveHrPolicy/<?=$user_id;?>",
						type: "POST",
						data: {
